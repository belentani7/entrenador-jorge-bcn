...
export const appRouter = router({
  ...
  leads: router({
    create: publicProcedure.input(z.object({ ...contactFields, goal: z.string().trim().min(2).max(500), preferredTime: z.string().trim().min(2).max(60) })).mutation(async ({ input }) => {
      const result = await createLead(input);
      const emailSent = await sendTransactionalEmail({ to: input.email, subject: "Hemos recibido tu mensaje · Jorge Performance", html: confirmationEmail(input.name) });
      return { success: true, id: result.id, emailSent } as const;
    }),
  }),
  bookings: router({
    create: publicProcedure.input(z.object({ ...contactFields, date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), time: z.string().regex(/^\d{2}:\d{2}$/) })).mutation(async ({ input }) => {
      if (await hasBooking(input.date, input.time)) throw new Error("Ese horario ya está reservado. Elige otro momento.");
      const result = await createBooking({ ...input, status: "confirmed" });
      const notified = await notifyOwner({ title: "Nueva reserva de sesión de prueba", content: `${input.name} ha solicitado una sesión el ${input.date} a las ${input.time}. Email: ${input.email}. Teléfono: ${input.phone}.` });
